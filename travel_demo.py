...
class NotablePlaceMention(BaseModel):
    ...
    description: str = Field(
        description="one to two sentence description of the place and why it is well known / why people love it"
    )
    place_type: str = Field(
        description="type of place / point of interest, e.g. restaurant, landmark, transportation, shopping, etc."
    )
    best_known_for: List[str] = Field(
        description="list of things this place is know for"
    )
    url: str = Field(
        description="link to the source youtube video"
    )
    scene: int = Field(
        description="relevant video scene number in source video, only provide an integer number"
    )
    timestamp: int = Field(
        description="relevant timestamp of scene source video, only provide an integer number"
    )
...
class NotablePlaceMentionsSummary(BaseModel):
    ...
    summary: str = Field(
        description="High level summary / commentary on the places retrieved and how relevant to the query"
    )
...
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        if message["role"] == "user":
            ...
            if str(type(response.response)) == "<class '__main__.NotablePlaceMentionsSummary'>":
                ...
                for m in response.response.place_mentions:
                    ...
                    st.markdown(t)
                    if m.url:
                        split = m.url[len('https://www.youtube.com/watch?v='):] + '_split_' + f'{m.scene:05d}'
                        col1, col2 = st.columns(2)
                        with col1:
                            st.image(f"https://storage.cloud.google.com/kdr-public/pandas70m/howto-travel/img/clip-start/{split}.jpg")
                            st.text("clip preview")
                        with col2:
                            st.video(m.url, format="video/mp4", start_time=m.timestamp)
